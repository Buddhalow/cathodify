  ...
  <div>
    <div v-if="skin" :style="{position: 'relative'}">
      ...
      <div v-bind:class="[crt ? 'crt' : '', flicker ? 'crt-flickering' : '']" >
        <iframe :width="skin.formats[format].width" :height="skin.formats[format].height" v-bind:style="{position: 'absolute', left: skin.formats[format].left, top: skin.formats[format].top, 'filter': 'saturate(' + saturate + '%)', opacity: skin.opacity}" :src="url + '?playsinline=1&autoplay=1'" frameborder="0" allow="encrypted-media"></iframe>
      </div>
